...
class StudentsGraph(GraphABC):
    ...
    def _overlap(self, student_neighbors):
        overlap = {}

        for student_u, neighbors_u in student_neighbors.items():
            for student_v, neighbors_v in student_neighbors.items():

                if student_u == student_v:
                    continue

                if student_u not in overlap:
                    overlap[student_u] = {}

                overlap[student_u][student_v] = len(neighbors_u & neighbors_v) / len(neighbors_u)
